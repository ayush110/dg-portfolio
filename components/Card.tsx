...
const Card = ({ title, description, imgSrc, skills, href, github }) => (
  <div className="md max-w-[544px] p-4 ">
    ...
    >
      ...
      <div className="p-6">
        <h2 className="mb-3 text-lg  leading-8 tracking-tight">
          {href ? (
        ...
        <div className=" mt-[12px]  flex-wrap items-center ">
          {skills?.map((skill) => (
            <div
              key={skill}
              className=" text-custom_black-800 border-custom_black-500 mr-4 mb-2 inline-block rounded-full   border py-[2px] px-[12px] text-xs"
            >
              <p>{skill}</p>
            </div>
          ))}
